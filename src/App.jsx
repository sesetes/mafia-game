...
function App() {
  ...
  const startGame = async () => {
    ...
    await updateDoc(doc(db, "rooms", roomDocId), { phase: "role_reveal", roles: assigned, alive: players, nightActions: { mafia: '', doctor: '', detective: '' }, votes: {}, targetTime: Date.now() + 12000, actedPlayers: [], mafiaChat: [], globalChat: [], defendingPlayer: '', nightCount: 1 });
  }

  const restartGame = async () => {
    await updateDoc(doc(db, "rooms", roomDocId), { phase: "waiting", alive: players, roles: {}, nightActions: { mafia: '', doctor: '', detective: '' }, votes: {}, nightLog: '', winner: '', targetTime: 0, actedPlayers: [], mafiaChat: [], globalChat: [], defendingPlayer: '', nightCount: 1 });
  }
  ...
  const performVoteResolution = async (docId, data) => {
    ...
    if (isTie || accused === 'skip' || accused === '') await updateDoc(doc(db, "rooms", docId), { phase: 'defense_result', nightLog: '⚖️ الأغلبية اختارت التخطي أو تعادلت الأصوات.. لا إعدام اليوم!', targetTime: Date.now() + 8000, votes: {}, actedPlayers: [] });
  ...
  const performDefenseResolution = async (docId, data) => {
    ...
    const winState = checkWin(newAlive, data.roles);
    await updateDoc(doc(db, "rooms", docId), { alive: newAlive, phase: winState ? 'game_over' : 'defense_result', nightLog: eliminated ? `🔨 انتهى الدفاع ولم يقتنعوا! تم إعدام (${eliminated}) 💀` : '🛡️ نجح الدفاع! تم سحب الأصوات ونجا المتهم.', winner: winState || '', votes: {}, targetTime: winState ? 0 : Date.now() + 8000, actedPlayers: [], defendingPlayer: '' });
  }
      ...
      interval = setInterval(async () => {
        ...
        if (left <= 0) {
          ...
          if (players[0] === playerName) {
            ...
            let data; docSnap.forEach(d => data = d.data());
            if (data.phase === phase) { 
              if (phase === 'role_reveal' || phase === 'defense_result') triggerNight(roomDocId, data.nightCount || 1);
              else if (phase === 'night') performNightResolution(roomDocId, data);
              else if (phase === 'day_result') triggerVoting(roomDocId);
    ...
    <div className="game-container">
      {!inRoom ? (
        ...
        <div>
          ...
          {myRole === 'مافيا' && phase !== 'waiting' && phase !== 'game_over' && phase !== 'role_reveal' && myMafiaMates.length > 0 && (
